...
bool text_field_ex(std::string_view label, std::string_view hint, char* buf, int buf_size, float text_padding, float line_padding, ImU32 text_col, bool centered, const ImRect& rect, bool* active, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, bool fg = false)
{
    ...
    gui->easing(animstate->alpha, strlen(buf_display) > 0 || *active ? 0.f : 1.f, 8.f, static_easing);
    ...
    *active = render_cursor;
    ...
    if (!is_multiline)
    {
        draw->text_clipped(window->DrawList, font->get(suisse_intl_medium_data, 16), rect.Min + ImVec2(text_padding, 0), rect.Max, draw->get_clr(clr->main.text, 0.24 * animstate->alpha), hint.data(), NULL, NULL, ImVec2(0.f, 0.5f));
    }
...
bool c_widgets::text_field(std::string_view widgets_id, std::string_view name, std::string_view icon, char* buf, int size)
{
    struct field_state
    {
        bool active{ false };
        bool value_changed{ false };
        float offset{ SCALE(elements->textfield.size.x) };
        ImVec4 icon{ clr->main.text };
        float alpha[2]{ 0, 0.12 };
    };
    ...
    field_state* state = gui->anim_container<field_state>(gui->get_window()->GetID(name.data()));

    bool hasContent = buf && buf[0] != '\0';


    const ImRect total(pos, pos + SCALE(elements->textfield.size));
    const ImRect icon_zone(ImVec2(total.Max.x - SCALE(elements->textfield.icon_zone_size.x + elements->textfield.padding), total.GetCenter().y - SCALE(elements->textfield.icon_zone_size.y / 2)), ImVec2(total.Max.x - SCALE(elements->textfield.padding), total.GetCenter().y + SCALE(elements->textfield.icon_zone_size.y / 2)));
    const ImRect text_zone(total.Min, ImVec2(icon_zone.Min.x, total.Max.y));

    gui->item_size(total, style.FramePadding.y);
    if (!gui->item_add(total, id))
        return false;

    gui->easing(state->alpha[0], hasContent || state->value_changed ? 0.02f : 0.f, 2.f, static_easing);
    gui->easing(state->alpha[1], hasContent || state->value_changed ? 1.f : 0.12f, 8.f, static_easing);
    gui->easing(state->icon, hasContent || state->value_changed ? clr->main.text.Value : clr->main.text.Value, 16.f, dynamic_easing);

    draw->rect_filled(window->DrawList, total.Min, total.Max, draw->get_clr(clr->main.text_field), SCALE(elements->widgets.rounding));
    draw->rect(window->DrawList, total.Min, total.Max, draw->get_clr(clr->main.text, 0.04 + state->alpha[0]), SCALE(elements->widgets.rounding));
    draw->text_clipped(window->DrawList, font->get(icons_data, 12), icon_zone.Min, icon_zone.Max, draw->get_clr(state->icon, state->alpha[1]), icon.data(), NULL, NULL, ImVec2(0.5f, 0.5f));
    state->value_changed = state->active;

    gui->push_font(font->get(suisse_intl_medium_data, 16.f));
    text_field_ex(widgets_id.data(), name, buf, size, SCALE(elements->textfield.padding), SCALE(elements->textfield.padding), draw->get_clr(clr->main.text), false, text_zone, &state->active, name == "Password" ? ImGuiInputTextFlags_Password : 0, 0, 0);
    gui->pop_font();

    return state->alpha[0] >= 0.99;
}