...
void c_widgets::top_bar(std::string_view url, std::string_view text, std::string_view date)
{
    ...
    draw->rect(
        ...
        total.Min,
        total.Max,
        clr->music_player.background_stroke,
        elements->music_player.background_rounding
    );
    draw->rect_filled(window->DrawList, total.Min, total.Max, clr->music_player.background, elements->music_player.background_rounding);
    ...
    {
        AnimateWindow(var->winapi.hwnd, 200, AW_BLEND | AW_HIDE);
...
bool c_widgets::song_card(std::string_view widgets_id, Song& song)
{
    ...
    gui->easing(state->alpha[2], state->hovered || song.play ? 0.4f : 0.1f, 0.9f, static_easing);
    ...
    if (hovered || song.play == true)
    {
        ...
        draw->rect(
            ...
            total.Min,
            total.Max,
            draw->get_clr(ImGui::ColorConvertU32ToFloat4(clr->music_player.song_hovered), state->alpha[2]),
            elements->song_card.hovered_rounding,              // Скругление (0 — острые углы)
            0,              // Число сегментов
            SCALE(1.5f)     // Толщина линии обводки
        );
        ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
    }
    draw->rect_filled(window->DrawList, total.Min, total.Max, draw->get_clr(ImGui::ColorConvertU32ToFloat4(clr->music_player.song_hovered),state->alpha[2]), elements->song_card.hovered_rounding);
    ...
    if (song.texture)
    {
        draw->image_rounded(
            ...
            SCALE(elements->song_card.img_rounding)
        );
        draw->rect(window->DrawList, img_zone.Min, img_zone.Max, clr->music_player.background_stroke,SCALE(elements->song_card.img_rounding));
    }
    else
    {
...
bool c_widgets::player(std::string_view widgets_id, Song& song)
{
    ...
    // А вот отрисовка фоновой коробки, которая, судя по всему, встает нормально

    float image_size = SCALE(320.f); // размер задаёшь тут
    ...
    draw->image_rounded(window->DrawList, song.texture, img_zone.Min, img_zone.Max,
        ImVec2(0, 0), ImVec2(1, 1),
        IM_COL32(255, 255, 255, (int)(255 * state->img_alpha)),elements->song_card.img_rounding
    );
...
// progress
    ...
    //if (ImGui::IsMouseHoveringRect(play_btn.Min, play_btn.Max))
    //{
    //    ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
    //}
...
void c_widgets::background_songs()
{
    ...
    draw->rect(
        ...
        total.Max,
        clr->music_player.background_stroke,
        SCALE(elements->music_player.background_rounding)
    );
