...
void c_gui::drag_window()
{
    ImVec2 pos = ImGui::GetWindowPos();
    ImVec2 mouse = ImGui::GetIO().MousePos;

    if (mouse.x >= pos.x && mouse.x <= pos.x + var->window.size.x &&
        mouse.y >= pos.y && mouse.y <= pos.y + SCALE(30) &&
        ImGui::IsMouseClicked(0))
    {
        ReleaseCapture();
        SendMessage(var->winapi.hwnd, WM_NCLBUTTONDOWN, HTCAPTION, 0);
    }
}
...
//        return;
//
//    if (ImRect{ gui->window_pos(), gui->window_pos() + SCALE(var->window.new_width, elements->top_bar.height + elements->window.padding.y) }.Contains(GImGui->IO.MousePos) && gui->mouse_clicked(mouse_button_left))
//        var->gui.dragging = true;
//
//    if (!gui->mouse_down(mouse_button_left))
//        var->gui.dragging = false;
//
//    if (var->window.size.x == var->window.new_width)
//        var->gui.update_size = true;
//
//    if (var->gui.update_size || var->gui.dragging)
//    {
//        GetWindowRect(hwnd, &rc);
//        MoveWindow(hwnd, rc.left + GetMouseDragDelta().x, rc.top + GetMouseDragDelta().y, GetWindowSize().x, SCALE(var->window.default_size.y), FALSE);
...
void c_gui::move_window(HWND hwnd, RECT rc)
{
    ...
    int width = (int)SCALE(var->window.size.x);
    int height = (int)SCALE(var->window.size.y);

    int center_x = rc.left + (rc.right - rc.left) / 2;
    int center_y = rc.top + (rc.bottom - rc.top) / 2;

    if (var->gui.dragging && gui->mouse_down(mouse_button_left))
    {
        center_x += (int)GetMouseDragDelta().x;
        center_y += (int)GetMouseDragDelta().y;
    }
...
struct text_state {
    ...
    int text_end = 0;
    std::vector<bool> is_original_char;
    std::vector<DWORD> dwLastUpdate;
    DWORD dwGlobalTimer = GetTickCount();
    bool finished = false;
};

std::unordered_map<int, text_state> texts;

char random_char()
{
    const char charset[] = "!@#$%^&*()_+[]/.,QwErTyU8iOpAsDfGhJkLzXcVbNnM-=";
    return charset[std::rand() % (sizeof(charset) - 1)];
}

std::string update_text(const ImGuiID id, const std::string& new_text, bool active, float delay)
{
    auto& state = texts[id];
    DWORD dwCurrentTime = GetTickCount();

    if (state.finished || state.updated_text.size() != new_text.size()) {
        state.updated_text = new_text;
        state.text_end = 0;
        state.is_original_char.assign(new_text.size(), false);
        state.dwLastUpdate.assign(new_text.size(), 0);
        state.dwGlobalTimer = dwCurrentTime;
        state.finished = false;
    }

    if (!state.finished) {
        float progress = static_cast<float>(dwCurrentTime - state.dwGlobalTimer) / delay;
        progress = progress > 1.0f ? 1.0f : progress;

        state.text_end = static_cast<int>(progress * new_text.size());

        if (progress >= 1.0f && active) {
            state.finished = true;
        }
    }
    ...
    for (size_t i = 0; i < new_text.size(); ++i) {
        if (new_text[i] == '\n') {
            state.updated_text[i] = '\n';
            state.is_original_char[i] = true;
        }
        else if (i < state.text_end) {
            state.updated_text[i] = new_text[i];
            state.is_original_char[i] = true;
        }
        else if (!state.is_original_char[i] && (state.dwLastUpdate.size() > i) &&
            (dwCurrentTime - state.dwLastUpdate[i] > 70)) {
            state.updated_text[i] = random_char();
            state.dwLastUpdate[i] = dwCurrentTime;
        }
    }

    return state.updated_text;
}

std::string c_gui::language(std::string_view eng, std::string_view rus, bool anim)
{
    std::string translated_text = var->gui.lang_count == 0 ? eng.data() : rus.data();
    const ImGuiID id = gui->get_window()->GetID(translated_text.data());

    return anim ? update_text(id, translated_text, var->gui.lang_changing, 1000) : translated_text;
}

std::vector<std::string> c_gui::wrap_text(ImFont* font, float max_width, const std::string& text)
{
...
//circle
static inline float loading_inv_length(const ImVec2& v)
{
    float len = sqrt(v.x * v.x + v.y * v.y);
    return 1.0 / ImMax(len, 0.0001f);
}

static inline float loading_field_fast(const ImVec2& frag, const ImVec2& resolution, float time)
{
    ImVec2 p = (frag - resolution * 0.5f) / resolution.y * 4.0f;

    float d = 0.0f;

    d += loading_inv_length(p + ImVec2(sinf(time), cosf(time)));

    for (float i = 0.0; i < 6.0f; i += 1.256f)
        d += loading_inv_length(p + ImVec2(sinf(i - time), cosf(i - time)));
    d = d * 0.75f - 6.0f;

    return d;
}
static inline float loading_field_aa_fast(const ImVec2& frag, const ImVec2& resolution, float time)
{
    const float d = loading_field_fast(frag, resolution, time);

    const float dx = fabsf(loading_field_fast(frag + ImVec2(1.0f, 0.0f), resolution, time) - d);
    const float dy = fabsf(loading_field_fast(frag + ImVec2(0.0f, 1.0f), resolution, time) - d);

    float fw = dx + dy;
    fw = ImClamp(fw, 0.0001f, 1.0f);

    return ImClamp(d / ImMin(1.0f, fw), 0.0f, 1.0f);
}
...
void c_gui::loading()
{
    struct loading_state
    {
        ...
        float close_alpha[3]{ 0.f,0.24f,0.06f };
        float timer{ 0.f };
        bool clicked{ false };
        float check_alpha{ 0.f };
        bool finished{ false };
        float finished_timer{ 0.f };
    ...
    ImVec2 panel_size = SCALE(560.f, 320.f);
    ImVec2 panel_min = pos + ImVec2((size.x - panel_size.x) * 0.5f, (size.y - panel_size.y) * 0.5f);
    ImVec2 panel_max = panel_min + panel_size;

    const ImRect panel(panel_min, panel_max);
    const ImRect fx_rect(panel.Min + SCALE(18.f, 54.f), panel.Max - SCALE(18.f, 58.f));

    loading_state* state = gui->anim_container<loading_state>(gui->get_window()->GetID("loading"));

    gui->easing(state->panel_alpha, var->gui.loading ? 1.f : 0.f, 10.f, static_easing);
    gui->easing(state->check_progress, state->finished ? 1.f : 0.f, 3.f, static_easing);
    
    if (!var->gui.loading && state->panel_alpha <= 0.01f)
    {
        state->timer = 0.f;
        state->clicked = false;
        state->check_alpha = 0.f;
        state->finished = false;
        state->close_alpha[0] = 0.f;
        state->close_alpha[1] = 0.24f;
        state->close_alpha[2] = 0.06f;
        state->finished_timer = 0.0f;
        state->check_progress = 0.0f;
        return;
    }

    if (var->gui.loading)
        state->timer += gui->fixed_speed(1.f);
   
    if (state->timer > 10)
        var->gui.loading = false;

    state->clicked = var->gui.loading && state->timer > 0.15f && gui->mouse_clicked(mouse_button_left);
    if (state->clicked)
        var->gui.loading = false;


    const float time = (float)ImGui::GetTime() * 0.8f;
    const ImVec2 resolution = fx_rect.GetSize();
    const float cell = ImMax(4.0f, SCALE(5.f));

    draw->push_clip_rect(drawlist, fx_rect.Min, fx_rect.Max, true);
    for (float y = 0.0f; y < resolution.y; y += cell)
    {
        for (float x = 0.0f; x < resolution.x; x += cell)
        {
            const ImVec2 frag(x, y);
            float a = loading_field_aa_fast(frag, resolution, time);
            ...
            const ImVec2 center = fx_rect.Min + ImVec2(x + cell * 0.5f, y + cell * 0.5f);

            draw->circle_filled(
                drawlist,
                center,
                cell * (0.28f + a * 0.12f),
                draw->get_clr(clr->main.super, a * 0.90f),
                10
            );
        }
    }
